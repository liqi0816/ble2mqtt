...
class TuyaFingerBot(pyee.EventEmitter):
    ...
    async def listen_notification(self):
        stream = util.map_stream(self.client.start_notify_stream(TuyaFingerBot.CHAR_ID['notification']), lambda data: data[1])
        async for message_raw in util.merge_packets(stream):
            message = util.parse_message(message_raw, self.session)
            code = message['code']
            print(f'<7>{self.identifier} received {str(code)} {message["data"].hex("-")}')
            if message.get('update_session', False):
                self.emit('update_session')
            elif code == util.TuyaCode.FUN_SENDER_PAIR:
                self.emit('paired')

    def once_future(self, event):
        future = asyncio.Future()
        super().once(event, lambda *data: future.set_result(data))
        return future
    ...
    async def sync_session(self):
        ...
        '''
        if self.client.is_connected and self.session.is_ready():
            return
        self.session = util.TuyaSession(self.local_key)
        await self.client.connect()
        asyncio.create_task(self.listen_notification())
        await asyncio.gather(self.once_future('update_session'), self.send_request(util.create_device_info_request(self.session)))
        await asyncio.gather(self.once_future('paired'),
